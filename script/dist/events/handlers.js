import { previousButtonNavigation, nextButtonNavigation, pokedexSearchForm, pokemonNameOrIdInput, favoriteStampButton, firstFavoriteButton, secondFavoriteButton, thirdFavoriteButton } from "../dom/elements.js";
...
export const setupButtonsAndFormEvents = () => {
    ...
    favoriteStampButton.addEventListener("click", () => {
        ...
        for (let i = 0; i < favorites.length; i = i + 1) {
            //avoid duplicates
            if (favorites[i] === state.currentID)
                return;
        ...
        if (index !== -1)
            favorites[index] = state.currentID;
        if (index === -1) {
            favorites.shift();
